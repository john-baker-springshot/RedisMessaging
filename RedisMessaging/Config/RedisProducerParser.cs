...
{
  public class RedisProducerParser : AbstractSingleObjectDefinitionParser
  {
    ...
    {
      var connectionPropName = nameof(RedisProducer.Connection);
      var connectionPropSchemaName = connectionPropName.ToCamelCase();

      var queuePropName = nameof(RedisProducer.Queue);
      var queuePropSchemaName = queuePropName.ToCamelCase();

      NamespaceUtils.CheckPresenceRule(element, parserContext, connectionPropName);
      
      NamespaceUtils.CheckAmbiguityRule(element, parserContext, connectionPropName);
      NamespaceUtils.CheckAmbiguityRule(element, parserContext, queuePropName);

      NamespaceUtils.AddConstructorArgIfAttributeOrElementDefined(element, parserContext, builder, connectionPropSchemaName);
      NamespaceUtils.AddConstructorArgIfAttributeOrElementDefined(element, parserContext, builder, queuePropSchemaName);
    }
