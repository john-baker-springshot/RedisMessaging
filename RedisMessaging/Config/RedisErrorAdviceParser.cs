...
{
  public class RedisErrorAdviceParser : AbstractSingleObjectDefinitionParser
  {
    ...
    {
      if (element.GetAttributeValueForProperty(nameof(ErrorAdvice.AdviceType)) == AdviceType.RetryRequeue.ToString() &&
        (element.HasAttributeForProperty(nameof(ErrorAdvice.RetryCount)) || element.HasAttributeForProperty(nameof(ErrorAdvice.RetryInterval))))
      {
        parserContext.ReaderContext.ReportFatalException(element, "Attributes retryCount and retryInterval are not applicable with adviceType: RetryRequeue.");
      }

      NamespaceUtils.SetValueIfAttributeDefined(builder, element, nameof(ErrorAdvice.AdviceType));
      NamespaceUtils.SetValueIfAttributeDefined(builder, element, nameof(ErrorAdvice.RetryOnFail));
      NamespaceUtils.SetValueIfAttributeDefined(builder, element, nameof(ErrorAdvice.ExceptionType));

      NamespaceUtils.SetValueIfAttributeDefined(builder, element, nameof(ErrorAdvice.RetryCount));
      NamespaceUtils.SetValueIfAttributeDefined(builder, element, nameof(ErrorAdvice.RetryInterval));
    }
